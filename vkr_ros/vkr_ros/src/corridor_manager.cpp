  ...
  : Node("corridor_manager", options)
{
  ...
  max_uavs_ = get_parameter("max_uavs").as_int();
...
void CorridorManager::checkAndFreeSegments()
{
  ...
  for (const auto& [uav_id, state] : uav_states_)
  {
    for (auto corridor_id : state.active_corridors)
    {
      ...
      {
        for (size_t i = 0; i < corridor_info.active_segments.size(); ++i)
        {
          auto segment_id = corridor_info.active_segments[i];
...
void CorridorManager::publishFreeEvents()
{
