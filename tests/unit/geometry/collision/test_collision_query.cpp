...
protected:
  ...
  std::shared_ptr<dynamic_obstacles::SweptOBBBVH> createDynamicObstacles() 
  {
    dynamic_obstacles::SweptOBBBVHBuilder builder(config_);
    
    auto now = std::chrono::steady_clock::now();
    
    dynamic_obstacles::OBB obb1;
    obb1.center = Eigen::Vector3f::Zero();
    obb1.half_extents = Eigen::Vector3f(5, 5, 5);
    ...
    builder.addWithTimeMapping(
      ...
      10,
      {
        Eigen::Vector3f(100, 100, 150), Eigen::Vector3f(200, 100, 150)
      },
      {
    ...
    dynamic_obstacles::OBB obb2;
    obb2.center = Eigen::Vector3f::Zero();
    obb2.half_extents = Eigen::Vector3f(10, 5, 3);
...
TEST_F(CollisionQueryTest, SetComponents) 
{
  ...
  auto dynamic_obs = createDynamicObstacles();
  collision_query_->setDynamicObstacles(dynamic_obs);
...
TEST_F(CollisionQueryTest, DynamicObstacleCollision) 
{
  auto dynamic_obs = createDynamicObstacles();
  collision_query_->setDynamicObstacles(dynamic_obs);
  ...
  Capsule capsule;
  capsule.p0 = Eigen::Vector3f(100, 100, 145);
  capsule.p1 = Eigen::Vector3f(100, 100, 155);
  capsule.radius = 1.0f;
  
  auto result = collision_query_->queryCapsule(capsule, now);
  
  EXPECT_TRUE(result.hit);
  EXPECT_EQ(result.object_type, HitResult::ObjectType::DYNAMIC_OBSTACLE);
  EXPECT_EQ(result.object_id, 10);
}

TEST_F(CollisionQueryTest, DynamicObstacleTimeInterval) 
{
  auto dynamic_obs = createDynamicObstacles();
  collision_query_->setDynamicObstacles(dynamic_obs);
  ...
  Capsule capsule;
  capsule.p0 = Eigen::Vector3f(150, 100, 145);
  capsule.p1 = Eigen::Vector3f(150, 100, 155);
  capsule.radius = 1.0f;
  
  auto result = collision_query_->queryCapsule(capsule, now, now + std::chrono::seconds(10));
...
TEST_F(CollisionQueryTest, CachingBehavior) 
{
  collision_query_->setCacheEnabled(true);
  ...
  auto result1 = collision_query_->queryCapsule(capsule, time);
  
  auto result2 = collision_query_->queryCapsule(capsule, time);
  
  EXPECT_EQ(result1.hit, result2.hit);
  
  auto result3 = collision_query_->queryCapsule(capsule, time, 1);
  EXPECT_EQ(result1.hit, result3.hit);
}

TEST_F(CollisionQueryTest, CacheDisabled) 
{
  collision_query_->setCacheEnabled(false);
  ...
  Capsule capsule;
  capsule.p0 = Eigen::Vector3f(50, 50, 100);
  capsule.p1 = Eigen::Vector3f(50, 50, 110);
  capsule.radius = 1.0f;
  
  for (int i = 0; i < 10; ++i) 
  {
    collision_query_->queryCapsule(capsule, std::chrono::steady_clock::now());
  }
...
TEST_F(CollisionQueryTest, EarlyExitOptimization) 
{
  ...
  EXPECT_EQ(stats_after.dynamic_checks, stats_before.dynamic_checks);
}
...
TEST_F(CollisionQueryTest, EarlyExitDisabled) 
{
  ...
  EXPECT_GT(stats_after.static_checks, stats_before.static_checks);
  EXPECT_GT(stats_after.dynamic_checks, stats_before.dynamic_checks);
}
...
TEST_F(CollisionQueryTest, JSONExport) 
{
  ...
  EXPECT_NE(content.find("\"dynamic_obstacles\": true"), std::string::npos);
