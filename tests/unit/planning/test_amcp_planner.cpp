...
TEST_F(AMCPlannerTest, PlanningWithTerrain) 
{
  ...
  PlanRequest request;
  request.start = Eigen::Vector3f(100, 100, 150);
  request.goal = Eigen::Vector3f(1000, 1000, 150);
  request.uav_id = 1;
  ...
  for (const auto& pos : result.path) 
  {
    ...
    {
      EXPECT_GE(altitude, config_.planning.min_clearance) 
          << "Point (" << pos.x() << ", " << pos.y() << ", " << pos.z() 
          << ") too close to ground at height " << ground_height;
    }
  }
  ...
  EXPECT_GE(min_altitude, config_.planning.min_clearance);
}
