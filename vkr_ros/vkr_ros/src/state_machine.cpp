...
void StateMachine::executeTakeoff()
{
  ...
  publishSetpoint(takeoff_position);
...
void StateMachine::executeFlying()
{
  ...
  {
    publishSetpoint(current_path_[path_index_]);
...
void StateMachine::executeLanded()
{
  if (!is_armed_)
  {
    ...
    transitionTo(State::IDLE);
    mission_.current_waypoint = mission_.waypoints.size();
  }
}
...
void StateMachine::executeReturnToBase()
{
  ...
  else
  {
    if (isAtPosition(mission_.base_position, 1.0f))
    {
      ...
      if (setLandMode())
      {
        RCLCPP_INFO(get_logger(), "Landing at base");
        transitionTo(State::IDLE);
      }
    }
  }
